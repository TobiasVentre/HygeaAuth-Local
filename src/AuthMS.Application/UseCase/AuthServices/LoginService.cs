...
namespace Application.UseCase.AuthServices
{
    public class LoginService : ILoginService
    {
            ...
            ITimeProvider timeProvider)
        {
            ...
            _maxFailedAccessAttempts = authTokenService.GetMaxFailedAccessAttempts();
            _lockoutDurationMinutes = authTokenService.GetLockoutDurationMinutes();
        }
        ...
        public async Task<LoginResponse> Login(LoginRequest request)
        {
            ...
            var refreshTokenLifetimeInMinutes = await _authTokenService.GetRefreshTokenLifetimeInMinutes();

            var refreshToken = new RefreshToken
            {
                Token = _refreshTokenHasher.Hash(rawRefreshToken),
                CreateDate = now,
                ExpireDate = now.AddMinutes(refreshTokenLifetimeInMinutes),
                UserId = user.UserId,
                IsActive = true,
                LastUsed = now
            };
